  ...
  try {
    ...
    for (let j = 0; j < estabelecimentosArray.length; j++) {
      ...
      try {
        ...
        console.log(`❌ Erro ao geocodificar ${estabelecimento.nome}: ${error instanceof Error ? error.message : 'Erro desconhecido'}`);
      }
    }
    ...
    const supermercadosComDados = await Promise.all(
      supermercadosNoRaio.map(async (supermercado) => {
        ...
        try {
          ...
          console.log(`Erro ao normalizar nome do supermercado ${supermercado.nome}: ${error instanceof Error ? error.message : 'Erro desconhecido'}`);

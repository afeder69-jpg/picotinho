...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 3. Cache de notas por (cnpj, janela)
    ...
    // 4. Iterar precos_atuais paginado
    let totals = {
      analisados: 0,
      legitimos: 0,
      suspeitos: 0,
    ...
    while (page < maxPages) {
      ...
      for (const p of precos) {
        ...
        // Buscar EAN do master para tentativa por EAN
        ...
        outer: for (const n of notas) {
          ...
          // (C) fallback: descrição EXATA + valor (e qtd se possível)
          // Apenas quando não houve match por EAN.
          for (const it of itens) {
            if (
              descAlvo &&
              it.desc &&
    ...
    // 5. Resumo final: top R$ 3,85 e replicação cruzada
    const { data: top385 } = await supabase
      .from("precos_atuais_auditoria")
      .select("produto_nome, estabelecimento_nome, valor_unitario, classificacao, motivo")
      .eq("job_id", jobId)
    ...
    const { data: topReplicados } = await supabase
      .from("precos_atuais_auditoria")
      .select(
        "produto_nome, estabelecimento_nome, valor_unitario, replicacao_count, classificacao, motivo",
      )
      .eq("job_id", jobId)
    ...
    return new Response(JSON.stringify({ ok: true, job_id: jobId, ...totals, resumo }), {

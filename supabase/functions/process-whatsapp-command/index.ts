  ...
  try {
    ...
    if (sessao) {
      ...
      if (!comandoExecutado) {
        ...
        const isConsultarEstoqueCompleto = textoNormalizado.match(/\b(consulta\s+estoque|estoque\s+completo)\b/) || textoNormalizado.trim() === 'estoque';
...
async function processarConsultarCategoria(supabase: any, mensagem: any): Promise<string> {
  try {
    ...
    const categoriaNome = categoriaEncontrada[0]?.categoria_nome;

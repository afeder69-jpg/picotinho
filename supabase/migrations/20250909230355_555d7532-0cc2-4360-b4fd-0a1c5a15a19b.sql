-- Remover o trigger problemático que está marcando produtos como manual antes da nota ser processada
DROP TRIGGER IF EXISTS atualizar_preco_usuario_automatico_trigger ON estoque_app;
DROP FUNCTION IF EXISTS atualizar_preco_usuario_inteligente();
...
-- Esta função verifica se produtos do estoque vieram de nota fiscal
...
BEGIN
    ...
    IF NEW.processada = true AND NEW.dados_extraidos IS NOT NULL AND 
       (OLD.processada != true OR OLD.dados_extraidos IS NULL) THEN

-- Função de normalização de telefone brasileiro
...
-- Trigger na tabela whatsapp_telefones_autorizados
CREATE TRIGGER trg_normalizar_telefone
BEFORE INSERT OR UPDATE OF numero_whatsapp ON public.whatsapp_telefones_autorizados
FOR EACH ROW EXECUTE FUNCTION public.normalizar_telefone_br();

-- Trigger na tabela whatsapp_configuracoes
CREATE TRIGGER trg_normalizar_telefone_config
BEFORE INSERT OR UPDATE OF numero_whatsapp ON public.whatsapp_configuracoes
FOR EACH ROW EXECUTE FUNCTION public.normalizar_telefone_br();
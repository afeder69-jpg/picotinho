-- Função para diagnosticar e corrigir inconsistências entre notas fiscais e estoque
...
BEGIN
    ...
    IF produtos_zerados > 0 THEN
        RETURN QUERY SELECT 
            ...
            format('%s produtos com quantidade zero encontrados', produtos_zerados),
    ...
    IF produtos_sem_preco > 0 THEN
        RETURN QUERY SELECT 
            ...
            format('%s produtos sem preço encontrados', produtos_sem_preco),
    ...
    RETURN QUERY SELECT 
        ...
        format('Diferença entre notas (%.2f) e estoque (%.2f)', total_notas, total_estoque),
        diferenca,
        CASE 
    ...
    IF ABS(diferenca) > 0.01 THEN
        ...
        RETURN QUERY SELECT 
            ...
            total_corrigido,
            format('Novo valor do estoque: %.2f (diferença: %.2f)', total_corrigido, total_notas - total_corrigido);
    END IF;

-- 🔒 SECURITY ENHANCEMENT: Complete security overhaul for profiles table

-- 1. Drop all existing policies first to remove dependencies
...
DROP FUNCTION IF EXISTS public.log_profile_access(uuid, text) CASCADE;
...
-- 4. Create security violation logging function
...
-- 5. Create new logging function for profile access
...
USING (
  auth.uid() = user_id
  AND public.secure_profile_access(user_id)
...
-- 8. Create trigger to prevent unauthorized changes
...
-- 10. Ensure user_id is always set
ALTER TABLE public.profiles 
ALTER COLUMN user_id SET NOT NULL;

-- 11. Create security indexes
...
-- 12. Grant minimal permissions
REVOKE ALL ON public.profiles FROM PUBLIC;
GRANT SELECT, INSERT, UPDATE ON public.profiles TO authenticated;

-- 13. Security documentation
COMMENT ON TABLE public.profiles IS 'Enhanced security: bulletproof RLS, comprehensive logging, data masking available';
COMMENT ON FUNCTION public.secure_profile_access(uuid) IS 'Multi-layer security validation with violation logging';
COMMENT ON FUNCTION public.log_security_violation(text, uuid, text) IS 'Security incident logging and alerting';
COMMENT ON FUNCTION public.mask_sensitive_profile_data(text, text, text) IS 'PII masking for safe data exposure';
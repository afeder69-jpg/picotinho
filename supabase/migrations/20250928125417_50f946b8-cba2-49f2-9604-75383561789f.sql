-- Corrigir função para não deletar produtos quando quantidade zera
...
BEGIN
    ...
    LOOP
        ...
        IF FOUND THEN
            ...
            -- Atualizar estoque (SEMPRE manter o produto, mesmo se quantidade = 0)
            UPDATE estoque_app 
            SET 
            ...
            RAISE NOTICE 'Estoque revertido: % (% - % = %) - PRODUTO MANTIDO', 
    ...
    RAISE NOTICE 'Reversão de estoque concluída para nota ID: % - PRODUTOS MANTIDOS COM QUANTIDADE ZERADA', OLD.id;
    RETURN OLD;
END;
$function$;

-- Criar trigger para executar automaticamente quando uma nota for deletada
CREATE OR REPLACE TRIGGER trigger_reverter_estoque_nota_excluida
    BEFORE DELETE ON notas_imagens
    FOR EACH ROW EXECUTE FUNCTION reverter_estoque_nota_excluida();
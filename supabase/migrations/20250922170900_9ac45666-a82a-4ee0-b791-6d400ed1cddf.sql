...
-- 3. Drop and recreate estoque_consolidado view with SECURITY INVOKER
DROP VIEW IF EXISTS public.estoque_consolidado;

CREATE VIEW public.estoque_consolidado
WITH (security_invoker = true)
AS
SELECT 
...
COMMENT ON VIEW public.estoque_consolidado IS 'Consolidated stock view with security invoker - respects user RLS policies';
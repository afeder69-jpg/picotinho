-- Melhorar função de consolidação para usar mesma lógica da normalização
...
BEGIN
    ...
    FOR produto_record IN 
        SELECT 
            user_id,
            -- Usar a mesma normalização do edge function
            UPPER(TRIM(
                REGEXP_REPLACE(
                    REGEXP_REPLACE(
                        REGEXP_REPLACE(
        ...
        HAVING COUNT(*) > 1
    LOOP
        -- Atualizar o primeiro produto com a quantidade consolidada e nome normalizado
        UPDATE estoque_app 
        SET 
        ...
        RAISE NOTICE 'Consolidado: % produtos em 1 - % -> Total: % %', 
            ...
            produto_record.nome_normalizado,
